...
double edgeGvalue=0, edgeHvalue=-1;
	...
	if (fgreater(maxSlope, 0, tolerance))
	{
		...
		if (openClosedList.OpenSize() != 0)
		{
			...
			else if (policy == DWP)
			{
				...
				if(fgreater(maxSlope, data.back().slope) || data.size() == 0){
					edgeHvalue = theHeuristic->HCost(openClosedList.Lookup(nodeid).data, neighbors[which]);
					edgeGvalue = edgeCosts[which];
